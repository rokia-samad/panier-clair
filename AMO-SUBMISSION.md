...
3. Incrémenter `version` dans `manifest.json`, mettre à jour la version du popup, puis exécuter `node scripts/build.mjs` et les vérifications de [ARCHITECTURE.md](ARCHITECTURE.md).
...
5. Créer aussi l’archive des sources avec `node scripts/package-source.mjs`. Le fichier `SOURCE-README.md` explique aux réviseurs comment reproduire `content.js`.
